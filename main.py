...
def main() -> None:
    ...
    st.set_page_config(
        page_title="Agentic File Management System",
        layout="wide",
    )
    ...
    if not st.session_state.authenticated:
        ...
        st.markdown(
            ...
            <div class="login-container">
                <h2 style="color: #1c83e1; margin-bottom: 20px;">
                    Secure Portal Login
                </h2>
            </div>
            """,
        ...
        with col_l2:
            ...
            if not app_settings.app_password:
                ...
                st.info(
                    "Set the AFMS_APP_PASSWORD environment "
    ...
    if "monitor" not in st.session_state:
        ...
        st.session_state.monitor = WorkspaceMonitor(
            db_manager,
    ...
    if page == "Home":
        ...
        render_header(
            "Agentic File System",
            "Operational status and real-time statistics",
        )

        with db_manager.connection() as conn:

            rows = FileRepository().counts_by_extension(
                conn
            )
        ...
        monitor_status = (
            "ACTIVE"
            if st.session_state.monitor.is_alive
            else "INACTIVE"
        )
        ...
        if rows:
            ...
            st.info(
                "No files scanned in database yet."
            )
        ...
        render_header(
            "Organize Workspace",
            "Categorize and relocate files using AI Reasoning",
        )
        ...
        if input_mode == "Local Directory on Computer":
            ...
            folder_path_str = st.text_input(
                "Select target folder to organize",
                value=str(
                    app_settings.project_root
                ),
            )
            ...
            dry_run_only = st.checkbox(
                "Dry Run Mode (simulate movements)",
                value=True,
            )
            ...
            if not dry_run_only:
                ...
                st.warning(
                    "Warning: Non-Dry Run mode will "
                    "physically move files on your disk. "
                    "Make sure you have a backup."
                )

                confirm_execute = st.checkbox(
                    "I confirm I want to execute these file moves.",
                    value=False,
                )
            ...
            if st.button(
                ...
                if not confirm_execute:
                    ...
                    st.error(
                        "Please tick the confirmation "
                        "checkbox above to authorize "
                ...
                if not files:
                    ...
                    st.info(
                        "No loose files identified in this directory."
                    )

                    return

                st.write(
                    f"Found {len(files)} files to evaluate..."
                )
                    ...
                    if res["error"]:
                        ...
                        if res["status"] == "needs_review":
                            ...
                            if res.get("reasoning"):
                                ...
                                with st.expander(
                                    "Why was this decision rejected?"
                                ):
        ...
        render_header(
            "Agent Audit Trail",
            "Historical record of autonomous folder movements",
        )
        ...
        if actions:
            ...
            st.write(
                "Review recent moves and provide "
                "corrections if necessary:"
            )

            for row in actions:

                col_info, col_feed = st.columns(
                    [7, 3]
                )
            ...
            st.info(
                "No folder organization actions "
                "recorded yet."
            )
        ...
        render_header(
            "System Settings",
            "Configure application paths and monitoring options",
        )
        ...
        with col_m1:
            ...
            if st.button(
                ...
                st.success(
                    "Watchdog background thread spawned."
                )
        ...
        if st.button(
            ...
            st.success(
                "SQLite database indexes cleared."
            )
